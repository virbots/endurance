     ...
     stands.forEach(function(sr) {
         ...
         if (thatOrFaster.length == 0) {
            ...
            distanceNotMet.add(dist);
             ...
             let distRatio = 100.0 / dist;
             let maxRatio = Math.max(...thatOrFaster.map(e => window.fromMinSec(e.time) / window.fromMinSec(e.pace)));
             maxRatio *= distRatio;
             if (maxRatio >= 1.0) {
                ...
                sr.status = 100.0 * maxRatio; // // maxTime / window.fromMinSec(sr.time);
                distanceNotMet.add(dist);
             }
         }
         i++;
